...
if variables == 0:
    print("Warning: No variables inputted. The program will terminate. Make sure to name variables in", input_file, "before the first study.")
    with open(output_file, "a") as file:
...
if variables > max_variables:
    print("Warning: Too many variables. The program will terminate. We do not support over", max_variables, "variables.")
    with open(output_file, "a") as file:
...
if pooled_sample == 0:
    print('Warning: The pooled sample size is 0. The program will terminate. At least one sample size must be non-zero.')
    with open(output_file, "a") as file:
...
for i in range(studies):
    ...
    if rank_A != rank_aug:
        ...
        print(f'Make sure to split randomised studies into two: one for the intervention sample and one for the control sample.')
        with open(output_file, "a") as file:
    ...
    if rank_A < A.shape[0]:
        print(f'Warning: Study {i} has redundant constraints. Please remove them to ensure correctness and to increase efficiency.')
        with open(output_file, "a") as file:
...
print('The sum of weighted KL-distances of this distribution to sets of probability functions given by individual studies is:', distance)
print('This sum is what is being minimised. The last improvement to this sum was by '+ str(previous_distance - distance) + ' < ' + str(stopping) + ' = stopping.')
